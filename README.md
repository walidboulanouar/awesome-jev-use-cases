...
- [harshithsunku/learn-jev-end-to-end](https://github.com/harshithsunku/learn-jev-end-to-end) by @harshithsunku. Free course of 12 Python notebooks that puts Jev inside an LLM agent loop as router, tool-call guard (a `Choice` of allow, ask or block plus `Noul` checks), done gate and judge across 13 use cases, with every tool read-only or dry-run. The author reports, from one run on 2026-09-23, that Jev matched a frontier LLM (92%) on a 40-email 8-way `Choice` task at about 8x lower latency and 87x lower cost, while both LLMs were more confident than Jev on an easy 40-SMS scam task.
